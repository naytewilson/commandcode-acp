...
describe("event mapping", () => {
  ...
  it("tool lifecycle maps with stable ids", () => {
    ...
    assert.equal((q.updates[0] as { status: string }).status, "pending");
    assert.equal((q.updates[0] as { kind: string }).kind, "read");
    const r = mapCmdEvent("tool_running", { type: "tool_running", toolCallId: "c1", toolName: "read_directory" });
    assert.equal((r.updates[0] as { status: string }).status, "in_progress");
    ...
    assert.equal((c.updates[0] as { status: string }).status, "completed");
  });
  ...
  it("tool_hook_blocked -> failed update (fail-closed evidence)", () => {
    ...
    assert.equal((o.updates[0] as { status: string }).status, "failed");
  });
  ...
  it("tool_use blocks in message_update open tool calls", () => {
    ...
    assert.equal((o.updates[0] as { toolCallId: string }).toolCallId, "t1");
  });
