...
function log(level: Level, msg: string): void {
  if (LEVELS.indexOf(level) < LEVELS.indexOf((LEVELS as readonly string[]).includes(LOG_LEVEL) ? (LOG_LEVEL as Level) : "info")) return;
  process.stderr.write(`[commandcode-acp:${level}] ${msg}\n`);
}

function cmdVersion(): Promise<string> {
...
function modelOptions(): Array<{ value: string; name: string; description?: string | null }> {
...
function modeState(session: BridgeSession): acp.SessionModeState {
  return {
...
function promptTextOf(prompt: ReadonlyArray<{ type: string; text?: string }>): string {
  ...
  for (const b of prompt) {
    ...
    else if (b.type !== "text") log("warn", `unsupported prompt block ignored: ${b.type}`);
  }
  const text = parts.join("\n").trim();
  if (!text) throw new acp.RequestError(-32602, "prompt requires non-empty text");
  return text;
}
...
export function buildAgent(): ReturnType<typeof acp.agent> {
  return (
    acp
      ...
      .onRequest("session/load", async (ctx) => {
        await ensureCatalog();
        const p = ctx.params as { sessionId?: string; cwd?: string };
        const id = typeof p.sessionId === "string" ? p.sessionId : "";
        const existing = sessions.get(id);
        if (existing) {
          if (typeof p.cwd === "string" && p.cwd.trim()) existing.cwd = p.cwd;
        ...
        if (persisted) {
          ...
          const s = sessions.makeRestored(
            ...
            cwd,
            persisted.model ?? catalog.defaultModel ?? null,
            persisted.effort,
            mode,
          );
        ...
        if (!id.trim()) throw new acp.RequestError(-32602, "session/load requires a session id");
        const cwd = typeof p.cwd === "string" && p.cwd.trim() ? p.cwd : process.cwd();
      ...
      .onRequest("session/prompt", (ctx) =>
        handlePrompt(
          ctx.params as { sessionId: string; prompt: ReadonlyArray<{ type: string; text?: string }> },
          ctx.client as unknown as { notify: (m: string, p: unknown) => Promise<void> },
        ),
      )
