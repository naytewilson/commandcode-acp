...
export class CmdRunError extends Error {
  ...
  constructor(classification: string, message: string, exitCode: number | null, cmdSessionId?: string) {
    super(message);
...
export function buildCmdArgs(o: {
  ...
  if (o.resumeCmdSessionId) args.push("--resume", o.resumeCmdSessionId);
...
/** Result of waiting for child exit. */
interface WaitResult {
  code: number | null;
  signal: NodeJS.Signals | null;
}

export function runCmdTurn(opts: RunOptions): { promise: Promise<RunOutcome>; cancel: () => Promise<void> } {
  let child: ChildProcess | undefined;
  let settled = false;
  const state = {
    cmdSessionId: undefined as string | undefined,
    unknownEventCount: 0,
    malformedLineCount: 0,
    totalBytes: 0,
    finalResult: null as CmdFinalResult | null,
  ...
  const promise = new Promise<RunOutcome>((resolve, reject) => {
    ...
    rl.on("line", (line: string) => {
      try {
        ...
        if (state.totalBytes > MAX_TOTAL_BYTES) {
          ...
          proc.kill("SIGTERM");
          finish(
            new CmdRunError("commandcode:general-error", "cmd NDJSON stream exceeded byte cap", null, state.cmdSessionId),
          );
          return;
        }
        ...
        if (isFinalResult(parsed)) {
          ...
          if (parsed.sessionId) state.cmdSessionId = parsed.sessionId;
          return;
        }
        const ev = parsed.event;
        const outcome = mapCmdEvent(ev.type, ev as unknown as Record<string, unknown>);
        ...
        for (const u of outcome.updates) onUpdate(u);
      } catch (err) {
        state.malformedLineCount += 1;
    ...
    const finish = (err?: Error, outcome?: RunOutcome) => {
      if (settled) return;
      settled = true;
      try {
        rl.close();
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      if (stop !== null && classification !== "success") {
        ...
        finish(undefined, {
          stopReason: stop as RunOutcome["stopReason"],
          cmdSessionId: state.cmdSessionId,
          unknownEventCount: state.unknownEventCount,
          malformedLineCount: state.malformedLineCount,
        });
    ...
    proc.on("exit", (code: number | null, signal: NodeJS.Signals | null) => {
      const w: WaitResult = { code, signal };
      void w;
  ...
  const cancel = async (): Promise<void> => {
    ...
    if (opts.signal) opts.signal.cancelled = true;
