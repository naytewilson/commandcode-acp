...
export interface MapOutcome {
  updates: AcpUpdate[];
  /** cmd session id seen on run_start (early binding). */
  runStartSessionId?: string;
  /** true when the event was recognized (even if it emits nothing). */
  recognized: boolean;
}
...
export function mapToolKind(toolName: string): ToolKind {
  ...
  if (EXEC_TOOLS.has(n)) return "execute";
...
export function mapCmdEvent(eventType: string, event: Record<string, unknown>): MapOutcome {
  switch (eventType) {
    case "text_delta": {
      const delta = str(event["delta"]);
      if (delta === undefined) return { updates: [], recognized: true };
      return {
    ...
    case "tool_running": {
      ...
      if (!toolCallId) return { updates: [], recognized: true };
    ...
    case "message_update": {
      ...
      if (!Array.isArray(content)) return { updates: [], recognized: true };
      const updates: AcpUpdate[] = [];
